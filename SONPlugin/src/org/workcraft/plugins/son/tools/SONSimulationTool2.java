...
public class SONSimulationTool2 extends AbstractTool implements ClipboardOwner {
	...
	private boolean conToBlock = true;
	...
	protected void updateState()
	{
		...
		backwardButton.setEnabled(stepCount > 0 || branchStep > 0);
		forwardButton.setEnabled(branchTrace==null && trace != null && stepCount < trace.size() || branchTrace != null && branchStep < branchTrace.size());
	...
	private boolean quietStep() {
		...
		if (stepCount>=trace.size()) return false;
	...
	private void reset() {
		if (stepCount==0&&branchTrace==null) {
...
*/
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (column==0) {
				if (trace!=null&&row<trace.size()) {
					...
					while (stepCount>row&&work) work=quietStepBack();
					while (stepCount<row&&work) work=quietStep();

					updateState();
				}
			} else {
				if (branchTrace!=null&&row>=stepCount&&row<stepCount+branchTrace.size()) {

					boolean work=true;
					while (stepCount+branchStep>row&&work) work=quietStepBack();
					while (stepCount+branchStep<row&&work) work=quietStep();
					updateState();
				}
			}
		}
	...
	private final class TraceTableCellRendererImplementation implements TableCellRenderer {
		...
		boolean isActive(int row, int column) {
			if (column==0) {
				...
				if (branchTrace!=null&&row>=stepCount&&row<stepCount+branchTrace.size()) {
					return (row-stepCount)==branchStep;
				}
			}
			return false;
		}
	...
	private class TraceTableModel extends AbstractTableModel {
		...
		public int getRowCount() {
			...
			return Math.max(tnum, bnum+stepCount);
		}
		...
		public Object getValueAt(int row, int col) {
			if (col==0) {
				...
				if (branchTrace!=null&&row>=stepCount&&row<stepCount+branchTrace.size()) {
					return branchTrace.get(row-stepCount);
				}
			}
			return "";
		}
	...
	public void activated(GraphEditor editor) {
		...
		abstractGroups.addAll(bsonAlg.getAbstractGroups(net.getGroups()));
	...
	public void executeEvent(List<EventNode> syncList) {
	...
	public void mousePressed(GraphEditorMouseEvent e) {
		...
		Node node = HitMan.hitDeepest(e.getPosition(), e.getModel().getRoot(), new Func<Node, Boolean>() {
				...
				public Boolean eval(Node node) {
					...
					if(node instanceof VisualEventNode && simuAlg.isEnabled(((VisualEventNode)node).getMathEventNode(), syncSet, phases) && !reverse){
						return true;
					}
					if(node instanceof VisualEventNode && simuAlg.isUnfireEnabled(((VisualEventNode)node).getMathEventNode(), syncSet, phases) && reverse){
						return true;
					}
					return false;
		...
		if (node instanceof VisualEventNode && conToBlock){
			Collection<EventNode> enabledEvents = new ArrayList<EventNode>();
			EventNode event = ((VisualEventNode)node).getMathEventNode();

			if(reverse){
				for(EventNode enable : net.getEventNodes())
			...
			List<EventNode> minimalEvents = simuAlg.getMinimalExeResult(event, syncSet, enabledEvents);
			List<EventNode> minimalReverseEvents = simuAlg.getMinimalReverseExeResult(event, syncSet, enabledEvents);

			if(!reverse){
				List<EventNode> possibleEvents = new ArrayList<EventNode>();
				for(EventNode psbE : enabledEvents)
	...
	private void setErrNum(List<EventNode> runList, boolean reverse){
		if (ErrTracingDisable.showErrorTracing()){
			Collection<EventNode> abstractEvents = new ArrayList<EventNode>();
			//get high level events
			for(EventNode absEvent : runList){
				for(ONGroup group : abstractGroups){
		...
		return new Decorator() {
			...
			public Decoration getDecoration(Node node) {
				if(node instanceof VisualEventNode && conToBlock) {
					...
					} else if (branchTrace==null&&trace!=null&&stepCount<trace.size()) {
						eventId = trace.get(stepCount);
