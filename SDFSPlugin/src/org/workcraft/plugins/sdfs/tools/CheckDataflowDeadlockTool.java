...
import org.workcraft.plugins.sdfs.tasks.CheckDataflowDeadlockTask;
...
public class CheckDataflowDeadlockTool implements Tool {
	...
	private CheckDataflowDeadlockTask task;
	ProgressMonitor<? super MpsatChainResult> monitor;

	public CheckDataflowDeadlockTool(Framework framework, Workspace ws) {
	...
	public void run(WorkspaceEntry we) {
		task = new CheckDataflowDeadlockTask(we, framework);
		framework.getTaskManager().queue(task, "Checking dataflow for deadlocks",
