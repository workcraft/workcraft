...
public class TestTool extends AbstractGraphEditorTool implements Command {
    ...
    private void esitmationTest(SON net) {
        ...
        // try {
        // Interval result =
        // timeAlg.EstimateEndTime(bsonAlg.getInitial(net.getComponents()).iterator().next(),
        // null);
    ...
    /*
     * private void timeTest(SON net) { TimeAlg timeAlg = new TimeAlg(net);
     *
     * for (Node node : net.getComponents()) {
     * System.out.println(net.getNodeReference(node)); try { for (String str :
     * timeAlg.onConsistecy(node)) { System.out.println(str); } } catch
     * (InvalidStructureException e) { System.out.println("Structure error"); }
     * } }
     */

    /*
     * private void bhvTimeTest(SON net) { BSONAlg bsonAlg = new BSONAlg(net);
     *
     * Collection<ONGroup> upperGroups =
     * bsonAlg.getUpperGroups(net.getGroups()); Collection<ONGroup> lowerGroups
     * = bsonAlg.getLowerGroups(net.getGroups()); Map<Condition,
     * Collection<Phase>> phases = bsonAlg.getAllPhases();
     *
     * TimeAlg timeAlg = new TimeAlg(net);
     *
     * for (ONGroup group : upperGroups) { for (TransitionNode t :
     * group.getTransitionNodes()) {
     * System.out.println(net.getNodeReference(t)); for (String str :
     * timeAlg.bsonConsistency(t, phases)) { System.out.println(str); } } }
     *
     * for (ONGroup group : lowerGroups) { for (Condition c :
     * group.getConditions()) { if (net.getInputPNConnections(c).isEmpty()) {
     * System.out.println("ini: " + net.getNodeReference(c)); for (String str :
     * timeAlg.bsonConsistency2(c)) { System.out.println(str); } }
     *
     * if (net.getOutputPNConnections(c).isEmpty()) { System.out.println(
     * "fine: " + net.getNodeReference(c)); for (String str :
     * timeAlg.bsonConsistency3(c)) { System.out.println(str); } }
     *
     * } } }
     */
    ...
    private void dfsTest(SON net) {
        ...
        Collection<Path> result = alg.getPaths(alg2.getONInitial(g).iterator().next(),
                alg2.getONFinal(g).iterator().next(), net.getGroups().iterator().next().getComponents());
    ...
    private void abtreactConditionTest(SON net) {
        ...
        for (Node node : net.getComponents()) {
            for (Condition c : alg.getUpperConditions(node)) {
                System.out.println(
                        "abstract condition of   " + net.getNodeReference(node) + "  is  " + net.getNodeReference(c));
            }
        }
        System.out.println("********************");
    }
    ...
    /*
     * private void convertBlockTest(SONModel net, VisualSON vnet) { for (Node
     * node : net.getSONConnections()) { System.out.println("before " +
     * net.getName(node) + " parent " + node.getParent().toString() + " type = "
     * + ((SONConnection) node).getType()); } vnet.connectToBlocks();
     * System.out.println("node size =" + net.getComponents().size()); for (Node
     * node : net.getSONConnections()) { System.out.println("after " +
     * net.getName(node) + " parent " + node.getParent().toString() + " type = "
     * + ((SONConnection) node).getType()); } }
     */
    ...
    private void blockMathLevelTest(SON net, VisualSON vnet) {
         ...
         * System.out. println("visual connection size : " +
         * block.getVisualSONConnections().size()); }
         */
    ...
    private void mathLevelTest(SON net, VisualSON vnet) {
        ...
        /*
         * for (PageNode page : net.getPageNodes()) { System.out.println(
         * "page parent  " + page.getParent().toString()); }
         */
        /*
         * for (VisualONGroup vgroup: vnet.getVisualONGroups()) {
         * System.out.println(vgroup.toString()); System.out.println(
         * "Visual Page size = " + vgroup.getVisualPages().size());
         * System.out.println("Visual Condition size = " +
         * vgroup.getVisualConditions().size()); System.out.println(
         * "Visual Connection size = " +
         * vgroup.getVisualSONConnections().size()); System.out.println(
         ...
         * System.out.println("visual page parent  " +
         * page.getParent().toString()); }
         */
    }
