...
public class BFSEntireEstimationAlg extends DFSEstimationAlg {
    ...
    public BFSEntireEstimationAlg(SON net, Interval d, Granularity g, ScenarioRef s, boolean twoDir)
    ...
    private void addSuperNodes() throws TimeEstimationException {
        ...
        for (PlaceNode p : initial) {
            try {
                ...
                // set time interval
                if (p.getStartTime().isSpecified()) {
                    con.setTime(p.getStartTime());
                }
                    ...
                    if (nd.getStartTime().isSpecified()) {
                        ...
                        if (i != null) {
                            ...
                            throw new TimeEstimationException(net.getNodeReference(m) + ".finish ("
                                    ...
                                    + ".start (" + nd.getStartTime().toString() + ")");
                        }
                    }
                }
                for (Time nd : getCausalPostset(m, nodes)) {
