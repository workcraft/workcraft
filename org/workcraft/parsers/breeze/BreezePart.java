...
public class BreezePart implements BreezeDefinition
{
	...
	private final List<BreezePartReference> parts;

	public BreezePart(List<PortDeclaration> ports, List<ChannelDeclaration> channels, List<BreezePartReference> parts)
	{
		this.ports = ports;
	...
	public static BreezePart register(BreezeLibrary library, String name, List<PortDeclaration> ports, List<ChannelDeclaration> channels, List<RawBreezePartReference> parts)
	{
		BreezePart part = new BreezePart(ports, channels, resolve(library, parts));
		library.put(name, part);
		return part;
	}
	...
	public <Port> BreezeInstance instantiate(BreezeFactory<Port> factory, ParameterValueList parameters)
	{
		...
		for(BreezePartReference part : parts)
		{
			BreezeInstance<Port> instance = part.definition().instantiate(factory, part.parameters());
