...
public class ModelManager {
	...
	public ModelManager() {
		this.uuid_model_map = new Hashtable<UUID, Class<?>>();
		this.uuid_component_list_map = new Hashtable<UUID, LinkedList<Class<?>>>();
		this.uuid_tool_list_map = new Hashtable<UUID, LinkedList<Class<?>>>();
		this.model_list = new LinkedList<Class<?>>();
		this.multi_tool_list = new LinkedList<Tool>();
	}

	@SuppressWarnings("unchecked")
	public LinkedList<Class<?>> getComponentsByModelUUID(UUID uuid) {
		LinkedList<Class<?>> lst = this.uuid_component_list_map.get(uuid);
		if (lst!=null)
	...
	public LinkedList<Class<?>> getToolsByModelUUID(UUID uuid) {
		LinkedList<Class<?>> lst = this.uuid_tool_list_map.get(uuid);
		if (lst!=null)
			return (LinkedList<Class<?>>)lst.clone();
		else
			return null;
	...
	public LinkedList<Tool> getMultiModelTools() {
		return (LinkedList<Tool>)this.multi_tool_list.clone();
	}

	@SuppressWarnings("unchecked")
	public LinkedList<Class<?>> getModelList() {
		return (LinkedList<Class<?>>)this.model_list.clone();
	}

	public Class<?> getModelByUUID(UUID uuid) {
		return this.uuid_model_map.get(uuid);
	}
	...
	public void addModel(Class<?> cls) {
		try
		{
			...
			if (this.uuid_model_map.get(uuid)!=null) {
				System.err.println ("Duplicate model id ("+uuid.toString()+"), skipping");
				return;
			}
			this.model_list.add(cls);
			this.uuid_model_map.put(uuid, cls);
	...
	public void addComponent (Class<?> cls) {
		try
		{
			...
			LinkedList<Class<?>> list = this.uuid_component_list_map.get(uuid);

			if (list == null)
			{
				list = new LinkedList<Class<?>>();
				this.uuid_component_list_map.put(uuid, list);
			}
