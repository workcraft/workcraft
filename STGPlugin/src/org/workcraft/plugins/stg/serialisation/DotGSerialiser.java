...
public class DotGSerialiser implements ModelSerialiser {
	...
	private void writeMarking(Model model, Collection<Place> places, PrintWriter out) {
		...
		for (Place p: places) {
			...
			if (p instanceof STGPlace) {
				if ( ((STGPlace)p).isImplicit() ) {
					reference = "<" + NamespaceHelper.hierarchicalToFlatName(model.getNodeReference(model.getPreset(p).iterator().next())) + "," +
							NamespaceHelper.hierarchicalToFlatName(model.getNodeReference(model.getPostset(p).iterator().next())) + ">";
				} else
		...
		for (Place p : places) {
			if (p instanceof STGPlace)
				...
				capacity.append(" " + NamespaceHelper.hierarchicalToFlatName(model.getNodeReference(p)) + "=" + ((STGPlace)p).getCapacity());
		}
	...
	private void writePN(PetriNetModel net, PrintWriter out) {
		...
		for (Transition t : net.getTransitions())
			transitions.add(NamespaceHelper.hierarchicalToFlatName(net.getNodeReference(t)));
