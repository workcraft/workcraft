...
public class DotGProvider {
	...
	public File getDotG (Path<String> source) {
		...
		if (we != null) {
			STGModel model;
			if (we.getObject() instanceof STGModel)
				model = (STGModel)we.getObject();
			else if (we.getObject() instanceof VisualSTG)
				model =  (STGModel) ((VisualSTG)we.getObject()).getMathModel();
			else
