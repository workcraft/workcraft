...
import org.workcraft.plugins.xbm.converters.StgElementaryCycle;
...
public class XbmSignalSimulationTool extends StgSimulationTool {
    ...
    public void updateState(GraphEditor editor) {
        ...
        for (XbmSignal xbmSignal : conditionalValue.keySet()) {
            StgElementaryCycle elemCycle = converter.getRelatedStgElementaryCycle(xbmSignal);
    ...
    private JPanel createConditionalSignalSetters(GraphEditor editor) {
        ...
        for (Map.Entry<XbmSignal, Boolean> entry: conditionalValue.entrySet()) {
            ...
            value.addActionListener(event -> {
                StgElementaryCycle elemCycle = converter.getRelatedStgElementaryCycle(entry.getKey());
                if (value.isSelected()) {
